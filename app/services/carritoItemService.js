...
export const crearItem = async (carritosId, planesId, emprendimientosIds) => {
  ...
  for (const id of emprendimientosIds) {
    ...
    const esExacto = yaExiste?.emprendimientos.length === 1 && yaExiste.emprendimientos[0].id === id;
...
//delete todo
export const eliminarTodoDelCarrito = async (carritosId) => {
  if (!carritosId) throw new Error("ID de carrito inválido");

  try {
    const resultado = await prisma.carritosItems.deleteMany({
      where: { carritosId },
    });

    return {
      mensaje: `Se eliminaron ${resultado.count} ítems del carrito`,
      cantidad: resultado.count,
    };
  } catch (err) {
    console.error("❌ Error al vaciar carrito:", err);
    throw new Error("Error al eliminar los ítems del carrito");
  }
};


// get items
// GET /carrito/:id/items
export const getCarritoItems = async (carritosId) => {
  const id = Number(carritosId);
  if (isNaN(id)) throw new Error("ID de carrito inválido");
