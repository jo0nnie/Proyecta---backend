...
export const crearPago = async (usuarioId, { metodoPagoId, tarjetaTemporal }) => {
  ...
  for (const item of carrito.idCarritosItems) {
    ...
    for (const emprendimiento of item.emprendimientos) {
      const ultimoBoost = await prisma.boosteos.findFirst({
        where: { emprendimientoId: emprendimiento.id },
        orderBy: { fechaFin: 'desc' },
      });

      let fechaInicio = hoy;

      if (ultimoBoost && new Date(ultimoBoost.fechaFin) > hoy) {
        // extiende desde el ultimo boost activo
        fechaInicio = new Date(ultimoBoost.fechaFin);
      }

      const fechaFin = new Date(fechaInicio);
      fechaFin.setDate(fechaFin.getDate() + item.plan.duracionDias);
      if (ultimoBoost && ultimoBoost.activo) {
        await prisma.boosteos.update({
          where: { id: ultimoBoost.id },
          data: { activo: false },
        });
      }
